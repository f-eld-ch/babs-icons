...
import {
  ...
  mkdirSync, copyFileSync, symlinkSync, rmSync, writeFileSync,
} from "node:fs";
...
const flag = (f: string, def: string) => {
  const i = args.indexOf(f); return i !== -1 ? (args[i + 1] ?? def) : def;
};
...
function buildWriteEntries(): WriteEntry[] {
  ...
  for (const id of ids) {
    ...
    for (const lang of LANGS) {
      ...
      if (e) { sources[lang] = e; hashes[lang] = hashOf(e.path); }
    }
    ...
    if (identical) {
      ...
      for (const lang of presentLangs) {
        we.langs[lang] = {
          svgFileName: base, langDirName: base, isSymlinkInSvg: false,
...
interface DirMeta { num: string; names: Partial<Record<Lang, string>>; }
...
function buildHierarchy(): { tops: DirMeta[]; subs: Map<string, DirMeta[]> } {
  ...
  for (const lang of LANGS) {
    ...
    let topDirs: string[];
    try { topDirs = readdirSync(root).sort(); } catch { continue; }

    for (const top of topDirs) {
      const tp = join(root, top);
      try { if (!statSync(tp).isDirectory()) continue; } catch { continue; }
      const tn = dirNum(top);
      if (!tn || !ALLOWED.has(tn)) continue;

      let td = tops.find(t => t.num === tn);
      if (!td) { td = { num: tn, names: {} }; tops.push(td); }
      td.names[lang] = dirLabel(top);

      let subDirs: string[];
      try { subDirs = readdirSync(tp).sort(); } catch { continue; }
      const sdirs = subDirs.filter(e => { try { return statSync(join(tp, e)).isDirectory(); } catch { return false; } });
      ...
      for (const sd of sdirs) {
        ...
        if (!sd_) { sd_ = { num: key, names: {} }; subs.get(tn)!.push(sd_); }
      ...
      if (sub) {
        for (const sd of sub) {
          if (e.id.startsWith(sd.num) && !placed) {
            const arr = symsByPrefix.get(sd.num) ?? []; arr.push(e);
        ...
        if (e.id.startsWith(prefix)) {
          const arr = symsByPrefix.get(prefix) ?? []; arr.push(e);
...
function collectStats(entries: WriteEntry[]): Stats {
  const buckets: Record<DivBucket, string[]> = {
    ident: [], deDiff: [], frDiff: [], itDiff: [], all3: [], partial: [],
  };
  ...
  const bump  = (cat: string, key: DivBucket | "edge") => {
    if (!byCat.has(cat)) byCat.set(cat, { ident:0, deDiff:0, frDiff:0, itDiff:0, all3:0, partial:0, edge:0 });
    byCat.get(cat)![key]++;
  };
  ...
  for (const e of entries) {
    ...
    if (e.presentLangs.length < 3) {
      buckets.partial.push(e.id); bump(cat, "partial"); continue;
    }
    if (e.identical) {
      buckets.ident.push(e.id); bump(cat, "ident"); continue;
    }
    const uniq = new Set([h.de, h.fr, h.it]).size;
    if (uniq === 3)        { buckets.all3.push(e.id);   bump(cat, "all3"); }
    else if (h.fr === h.it) { buckets.deDiff.push(e.id); bump(cat, "deDiff"); }
    else if (h.de === h.it) { buckets.frDiff.push(e.id); bump(cat, "frDiff"); }
    else if (h.de === h.fr) { buckets.itDiff.push(e.id); bump(cat, "itDiff"); }
    else                   { edge.push({ id: e.id, pattern: "2-unique-unknown" }); bump(cat, "edge"); }
  }
...
function printStats(entries: WriteEntry[], stats: Stats) {
  ...
  console.log(`Symbol IDs: ${entries.length}  (categories: ${[...ALLOWED].sort(compareNumeric).join(",")})`);
  ...
  if (buckets.partial.length) {
    ...
    for (const id of buckets.partial) {
      const e = byId.get(id)!;
      console.log(`    ${id}  present: [${e.presentLangs.join("+")}]  missing: [${LANGS.filter(l => !e.presentLangs.includes(l)).join("+")}]`);
    }
  }
