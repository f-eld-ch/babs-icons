  ...
  for (const top of readdirSync(root).sort()) {
    ...
    if (subs.length === 0) {
      ...
      for (const sd of subs) {
        const sn = dirNum(sd);
        if (!sn) continue;
        for (const f of readdirSync(join(tp, sd))
          .sort()
 ...
 */
export function indexPatterns(
  lang: Lang,
  opts: IndexOptions,
): Map<string, Partial<Record<PatternVariant, SourceEntry>>> {
...
export function indexPatternsAll(
  opts: IndexOptions,
): Record<Lang, Map<string, Partial<Record<PatternVariant, SourceEntry>>>> {
  return {
...
export function allIds(indices: Record<Lang, Map<string, SourceEntry>>): string[] {
  return [...new Set([...indices.de.keys(), ...indices.fr.keys(), ...indices.it.keys()])].sort(
    compareNumeric,
  );
}
